...
class VirtualPTMMicroenv(nn.Module):
    def __init__(self, hidden_dim=128, num_envs=2):
        super(VirtualPTMMicroenv, self).__init__()
        self.num_envs = num_envs
        self.env_proj = nn.Sequential(nn.Dropout(0.3), nn.Linear(hidden_dim, hidden_dim * num_envs))
        self.drug_query = nn.Linear(hidden_dim, hidden_dim)

    def forward(self, p_feat, d_feat):
        batch_size = p_feat.size(0)
        envs = self.env_proj(p_feat).view(batch_size, self.num_envs, -1)
...
class VirtualPharmacophoreRouting(nn.Module):
    def __init__(self, hidden_dim=128, num_pharmas=2):
        super(VirtualPharmacophoreRouting, self).__init__()
        self.num_pharmas = num_pharmas
        self.pharma_proj = nn.Sequential(nn.Dropout(0.3), nn.Linear(hidden_dim, hidden_dim * num_pharmas))
        self.ptm_query = nn.Linear(hidden_dim, hidden_dim) 

    def forward(self, d_feat, p_feat):
        batch_size = d_feat.size(0)
        pharmas = self.pharma_proj(d_feat).view(batch_size, self.num_pharmas, -1)
...
class CA_HGT_Model(nn.Module):
    def __init__(self, metadata, K=2, M=2, d_in=768, p_in=1280, hidden_dim=128):
        ...
        self.vpma = VirtualPTMMicroenv(hidden_dim=hidden_dim, num_envs=K)
        self.vpr = VirtualPharmacophoreRouting(hidden_dim=hidden_dim, num_pharmas=M)
    ...
    def forward(self, x_dict, edge_index_dict, batch_d_idx, batch_p_idx):
        ...
        d_vpr_out, pharma_attn = self.vpr(D_raw, P_raw)
        p_vpma_out, env_attn = self.vpma(P_raw, D_raw)

        D_final = d_vpr_out + D_raw
        P_final = p_vpma_out + P_raw
        ...
        if not self.training:
            return logits, env_attn, pharma_attn
        return logits
...
class PBIFNetRunner:
    def __init__(self, clean_dir, split_dir, K=2, M=2, batch_size=128, lr=1e-4,
    ...
    def run_train(self, fold):
        ...
        model = CA_HGT_Model(graph.metadata(), K=self.K, M=self.M, hidden_dim=self.hidden_dim).to(self.device)
...
def main():
    parser = argparse.ArgumentParser(
        description="PBIFNet: A Pre-trained Language Model and Induced-Fit Mechanism-Based Framework for Drug-PTM Interaction Prediction"
    )
    ...
    parser.add_argument('-k', '--K', type=int, default=2,
                        help="Number of target virtual microenvironments (VPMA). Default is 2.")
    parser.add_argument('-m', '--M', type=int, default=2,
                        help="Number of drug virtual pharmacophores (VPR). Default is 2.")
    ...
    if len(fold_results) > 0:
        ...
        res_file = 'sota_km_full_metrics_results.csv'
        write_header = not os.path.exists(res_file)
        with open(res_file, 'a') as f_out:
            if write_header:
                f_out.write("K,M,Fold,AUC,AUPR,Accuracy,Precision,Recall,F1_score,MCC,Specificity\n")

            for r in fold_results:
                f_out.write(f"{args.K},{args.M},{r['Fold']},{r['AUC']:.4f},{r['AUPR']:.4f},{r['Accuracy']:.4f},{r['Precision']:.4f},{r['Recall']:.4f},{r['F1']:.4f},{r['MCC']:.4f},{r['Specificity']:.4f}\n")

            f_out.write(f"{args.K},{args.M},avg,{avg_res['AUC']:.4f},{avg_res['AUPR']:.4f},{avg_res['Accuracy']:.4f},{avg_res['Precision']:.4f},{avg_res['Recall']:.4f},{avg_res['F1']:.4f},{avg_res['MCC']:.4f},{avg_res['Specificity']:.4f}\n")
            f_out.write(f"{args.K},{args.M},std,{std_res['AUC']:.4f},{std_res['AUPR']:.4f},{std_res['Accuracy']:.4f},{std_res['Precision']:.4f},{std_res['Recall']:.4f},{std_res['F1']:.4f},{std_res['MCC']:.4f},{std_res['Specificity']:.4f}\n")
